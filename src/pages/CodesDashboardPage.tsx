...
const thClass =
  "p-4 px-5 text-left bg-[#F8FAFA] text-muted font-semibold text-sm border-b border-[#EEF2F2]";
const sortableThClass = `${thClass} cursor-pointer select-none`;
const tdClass = "p-4 px-5 text-sm text-heading";
const _actionBtnClass =
  "bg-transparent border-0 text-zinc-950 cursor-pointer p-0 font-sans text-sm";
const paginationBtnClass =
  "py-1.5 px-3.5 text-sm rounded-[8px] border border-[#d1d5db] bg-[#f3f4f6] cursor-pointer text-[#4b5563]";
...
export function CodesDashboardPage(): React.ReactElement {
  ...
  const [stickerValueEmbedCode, setStickerValueEmbedCode] =
    useState<ValueEmbedCodePrintData | null>(null);
  ...
  const [valueEmbedConfirm, setValueEmbedConfirm] = useState<{
    action: "redeem" | "cancel";
    row: ValueEmbedCodeSet;
  } | null>(null);
  ...
  const [valueEmbedSort, setValueEmbedSort] = useState<{
    column: ValueEmbedSortColumn;
    direction: SortDirection;
  }>({
    column: "createdAt",
    direction: "desc",
  });
  const [selfTitlingSort, setSelfTitlingSort] = useState<{
    column: SelfTitlingSortColumn;
    direction: SortDirection;
  }>({
    column: "createdAt",
    direction: "desc",
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event: MouseEvent): void => {
      if (
        selfTitlingMenuRef.current &&
        !selfTitlingMenuRef.current.contains(event.target as Node)
      ) {
  ...
  const sortValueEmbed = (
    ...
    return [...list].sort((a, b) => {
      ...
      if (col === "value") cmp = a.value - b.value;
        ...
        cmp = String(a[col as keyof ValueEmbedCodeSet] ?? "").localeCompare(
          String(b[col as keyof ValueEmbedCodeSet] ?? "")
        );
      return dir === "asc" ? cmp : -cmp;
    });
  };
  const sortSelfTitling = (
    list: SelfTitlingCodeSet[],
    col: SelfTitlingSortColumn,
    dir: SortDirection
  ): SelfTitlingCodeSet[] => {
    return [...list].sort((a, b) => {
      let cmp = 0;
  ...
  const sortedValueEmbedList = useMemo(
    ...
    [valueEmbedList, valueEmbedSort, sortValueEmbed]
  );
  ...
  const _getRowClass = (): string => "border-b border-[#EEF2F2] bg-white";
  ...
  return (
    <div className="min-w-0">
                  ...
                  >
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          navigate(pathToValueEmbedDetail(row.id));
                        }
                      }}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 gap-1.5 text-muted">
                        <span>
                          <span className="font-medium text-heading">Public code:</span>{" "}
                        ...
                        <span>
                          <span className="font-medium text-heading">Created:</span>{" "}
                    ...
                    <div
                      ref={valueEmbedMenuRowId === row.id ? valueEmbedMenuRef : undefined}
                    ...
                    >
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setValueEmbedMenuRowId((id) => (id === row.id ? null : row.id));
                        }}
                      ...
                      {valueEmbedMenuRowId === row.id && (
                        ...
                        >
                          <button
                            type="button"
                            role="menuitem"
                            className="w-full px-4 py-2 text-left text-sm text-zinc-950 hover:bg-zinc-100"
                            onClick={() => {
                              setEditValueEmbedRow(row);
                              setValueEmbedMenuRowId(null);
                            }}
                          >
                            Edit
                          ...
                          <button
                            ...
                            className="w-full px-4 py-2 text-left text-sm text-zinc-950 hover:bg-zinc-100"
                            onClick={() => {
                              setStickerValueEmbedCode({
                                publicCode: row.publicCode,
                                privateCode: row.privateCode,
                                qrUrl: row.qrUrl,
                              });
                          ...
                          <button
                            ...
                            className="w-full px-4 py-2 text-left text-sm text-zinc-950 hover:bg-zinc-100"
                            onClick={() => {
                              setValueEmbedConfirm({ action: "redeem", row });
                              setValueEmbedMenuRowId(null);
                            }}
                          >
                            Redeem
                          ...
                          <button
                            ...
                            className="w-full px-4 py-2 text-left text-sm text-zinc-950 hover:bg-zinc-100"
                            onClick={() => {
                              setValueEmbedConfirm({ action: "cancel", row });
                              setValueEmbedMenuRowId(null);
                            }}
                          >
                            Cancel
                  ...
                  <div className="flex items-center justify-between p-4 border-t border-[#EEF2F2] flex-wrap gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setValueEmbedPage((p) => Math.min(valueEmbedTotalPages, p + 1))
                        }
              ...
              <div className="hidden md:block overflow-x-auto">
                <div className="overflow-hidden border border-[#E8E8E8] rounded-b-card bg-white shadow-soft">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        <th
                          ...
                          onClick={() => handleValueEmbedSort("label")}
                          aria-sort={
                            valueEmbedSort.column === "label"
                              ? valueEmbedSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                          }
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleValueEmbedSort("publicCode")}
                          aria-sort={
                            valueEmbedSort.column === "publicCode"
                              ? valueEmbedSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleValueEmbedSort("value")}
                          aria-sort={
                            valueEmbedSort.column === "value"
                              ? valueEmbedSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleValueEmbedSort("status")}
                          aria-sort={
                            valueEmbedSort.column === "status"
                              ? valueEmbedSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleValueEmbedSort("createdAt")}
                          aria-sort={
                            valueEmbedSort.column === "createdAt"
                              ? valueEmbedSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleValueEmbedSort("expiration")}
                          aria-sort={
                            valueEmbedSort.column === "expiration"
                              ? valueEmbedSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                    ...
                    <tbody>
                      {paginatedValueEmbedList.map((row, _index) => (
                        <tr
                          key={row.id}
                          role="button"
                          tabIndex={0}
                        ...
                        >
                          ...
                          <td className={`${tdClass} w-0 pr-4`}>
                            ...
                            >
                              ...
                              <span
                                className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none w-4 h-4 shrink-0"
                                aria-hidden
                              >
                                <svg
                  ...
                  >
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          navigate(pathToSelfTitlingDetail(row.id));
                        }
                      }}
                    ...
                    >
                      ...
                      <div className="flex flex-col gap-2 min-w-0 flex-1">
                        ...
                        <div className="grid grid-cols-1 gap-1.5 text-muted">
                          ...
                          <span>
                            <span className="font-medium text-heading">UNS name:</span>{" "}
                            {row.unsName}
                          </span>
                          <span>
                            <span className="font-medium text-heading">Created:</span>{" "}
                    ...
                    <div
                      ref={selfTitlingMenuRowId === row.id ? selfTitlingMenuRef : undefined}
                      className="absolute top-4 right-4"
                    >
                      <button
                        type="button"
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelfTitlingMenuRowId((id) => (id === row.id ? null : row.id));
                        }}
                        className="p-1 rounded hover:bg-zinc-200 text-zinc-600 focus:outline-none focus:ring-2 focus:ring-zinc-400"
                        aria-label="Actions"
                  ...
                  <div className="flex items-center justify-between p-4 border-t border-[#EEF2F2] flex-wrap gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setSelfTitlingPage((p) => Math.min(selfTitlingTotalPages, p + 1))
                        }
              ...
              <div className="hidden md:block overflow-x-auto">
                <div className="overflow-hidden border border-[#E8E8E8] rounded-b-card bg-white shadow-soft">
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        <th className={thClass} style={{ width: 56 }}>
                          Image
                        </th>
                        <th
                          className={sortableThClass}
                          onClick={() => handleSelfTitlingSort("itemTag")}
                          aria-sort={
                            selfTitlingSort.column === "itemTag"
                              ? selfTitlingSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                          }
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleSelfTitlingSort("publicCode")}
                          aria-sort={
                            selfTitlingSort.column === "publicCode"
                              ? selfTitlingSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleSelfTitlingSort("unsName")}
                          aria-sort={
                            selfTitlingSort.column === "unsName"
                              ? selfTitlingSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleSelfTitlingSort("status")}
                          aria-sort={
                            selfTitlingSort.column === "status"
                              ? selfTitlingSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                        ...
                        <th
                          className={sortableThClass}
                          onClick={() => handleSelfTitlingSort("createdAt")}
                          aria-sort={
                            selfTitlingSort.column === "createdAt"
                              ? selfTitlingSort.direction === "asc"
                                ? "ascending"
                                : "descending"
                              : undefined
                    ...
                    <tbody>
                      {paginatedSelfTitlingList.map((row, _index) => (
                        <tr
                          key={row.id}
                          role="button"
                          tabIndex={0}
                          ...
                          <td className={`${tdClass} w-[56px] pl-4 pr-2 align-middle`}>
                            {row.imageUrl ? (
                              <img
                                src={row.imageUrl}
                                alt=""
                                className="w-10 h-10 rounded-lg object-cover border border-zinc-200 bg-zinc-100"
                              />
                            ) : (
                              <div
      ...
      <StickerPrintModal
        ...
        variant={activeTab}
        valueEmbedCode={
          activeTab === "value-embed" ? (stickerValueEmbedCode ?? undefined) : undefined
        }
      />
      <Modal
        ...
        title={
          valueEmbedConfirm?.action === "redeem"
            ? "Redeem code?"
            : valueEmbedConfirm?.action === "cancel"
              ? "Cancel / remove code?"
              : ""
        }
        size="confirmation"
              ...
              <Button
                onClick={async () => {
                  ...
                  if (valueEmbedConfirm.action === "redeem") {
                    navigate(
                      `${ROUTES.REDEEM}?code=${encodeURIComponent(valueEmbedConfirm.row.publicCode)}`
                    );
                  } else {
